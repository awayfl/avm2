...
export class b2PolygonShape extends b2Shape
{
	...
	public Set(other:b2Shape):void 
	{
		...
		if (other instanceof b2PolygonShape)
		{
			...
			for (var i:number /** int */ = 0; i < this.m_vertexCount; i++)
			{
	...
	public SetAsVector(vertices:Array<b2Vec2>, vertexCount:number = 0):void
	{
		...
		{
			var i1:number /** int */  = i;
			var i2:number /** int */  = i + 1 < this.m_vertexCount ? i + 1 : 0;
			var edge:b2Vec2 = b2Math.SubtractVV(this.m_vertices[i2], this.m_vertices[i1]);
...
//#endif
	...
	*/
	...
	public ComputeAABB(aabb:b2AABB, xf:b2Transform) : void
	{
		...
		for (var i:number /** int */  = 1; i < this.m_vertexCount; ++i)
		{
			tVec = this.m_vertices[i];
	...
	*/
			...
			c:b2Vec2):number
	{
		...
		var diveCount:number /** int */  = 0;
		var intoIndex:number /** int */  = -1;
		var outoIndex:number /** int */  = -1;
		...
		var intoIndex2:number /** int */  = (intoIndex + 1) % this.m_vertexCount;
		var outoIndex2:number /** int */  = (outoIndex + 1) % this.m_vertexCount;
		...
		var intoVec:b2Vec2 = new b2Vec2(this.m_vertices[intoIndex].x * (1 - intoLamdda) + this.m_vertices[intoIndex2].x * intoLamdda,
										this.m_vertices[intoIndex].y * (1 - intoLamdda) + this.m_vertices[intoIndex2].y * intoLamdda);
		var outoVec:b2Vec2 = new b2Vec2(this.m_vertices[outoIndex].x * (1 - outoLamdda) + this.m_vertices[outoIndex2].x * outoLamdda,
	...
	*/
	...
	{
		for (var i:number /** int */  = this.m_vertices.length; i < count; i++)
		{
	...
	public static ComputeOBB(obb:b2OBB, vs:Array<b2Vec2>, count:number /** int */ ) : void
	{
		...
		for (i = 1; i <= count; ++i)
		{
			...
			for (var j:number /** int */  = 0; j < count; ++j)
			{
				//b2Vec2 d = p[j] - root;
