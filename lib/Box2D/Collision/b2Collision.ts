...
export class b2Collision{
											...
											poly2:b2PolygonShape, xf2:b2Transform):number
	{
		...
		for (var i:number /** int */ = 0; i < count2; ++i)
		{
											...
											poly2:b2PolygonShape, xf2:b2Transform):number
	{
		...
		var edge:number /** int */ = 0;
		var maxDot:number = -Number.MAX_VALUE;
		for (var i:number /** int */ = 0; i < count1; ++i)
		{
		...
		var s:number = this.EdgeSeparation(poly1, xf1, edge, poly2, xf2);
		
		// Check the separation for the previous edge normal.
		var prevEdge:number /** int */ = edge - 1 >= 0 ? edge - 1 : count1 - 1;
		var sPrev:number = this.EdgeSeparation(poly1, xf1, prevEdge, poly2, xf2);
		
		// Check the separation for the next edge normal.
		var nextEdge:number /** int */ = edge + 1 < count1 ? edge + 1 : 0;
		var sNext:number = this.EdgeSeparation(poly1, xf1, nextEdge, poly2, xf2);
		
		// Find the best edge and the search direction.
		var bestEdge:number /** int */;
		var bestSeparation:number;
		var increment:number /** int */;
											...
											poly2:b2PolygonShape, xf2:b2Transform) : void
	{
		...
		var i1:number /** int */ = index;
		var i2:number /** int */ = i1 + 1 < count2 ? i1 + 1 : 0;
											...
											polyB:b2PolygonShape, xfB:b2Transform) : void
	{
		...
		var edgeA:number /** int */ = 0;
		b2Collision.s_edgeAO[0] = edgeA;
		var separationA:number = this.FindMaxSeparation(b2Collision.s_edgeAO, polyA, xfA, polyB, xfB);
		edgeA = b2Collision.s_edgeAO[0];
		if (separationA > totalRadius)
			return;

		var edgeB:number /** int */ = 0;
		b2Collision.s_edgeBO[0] = edgeB;
		var separationB:number = this.FindMaxSeparation(b2Collision.s_edgeBO, polyB, xfB, polyA, xfA);
		...
		var xf1:b2Transform;
		var xf2:b2Transform;
		var edge1:number /** int */;		// reference edge
		var flip:number /** uint */;
		...
		var incidentEdge:Array<ClipVertex> = b2Collision.s_incidentEdge; 
		...
		var localTangent:b2Vec2 = b2Collision.s_localTangent;
		localTangent.Set(local_v12.x - local_v11.x, local_v12.y - local_v11.y);
		localTangent.Normalize();
		
		var localNormal:b2Vec2 = b2Collision.s_localNormal;
		localNormal.x = localTangent.y;
		localNormal.y = -localTangent.x;
		
		var planePoint:b2Vec2 = b2Collision.s_planePoint;
		planePoint.Set(0.5 * (local_v11.x + local_v12.x), 0.5 * (local_v11.y + local_v12.y));
		
		var tangent:b2Vec2 = b2Collision.s_tangent;
		//tangent = b2Math.b2MulMV(xf1.R, localTangent);
		tMat = xf1.R;
		tangent.x = (tMat.col1.x * localTangent.x + tMat.col2.x * localTangent.y);
		tangent.y = (tMat.col1.y * localTangent.x + tMat.col2.y * localTangent.y);
		var tangent2:b2Vec2 = b2Collision.s_tangent2;
		tangent2.x = - tangent.x;
		tangent2.y = - tangent.y;
		var normal:b2Vec2 = b2Collision.s_normal;
		normal.x = tangent.y;
		normal.y = -tangent.x;

		//v11 = b2Math.MulX(xf1, local_v11);
		//v12 = b2Math.MulX(xf1, local_v12);
		var v11:b2Vec2 = b2Collision.s_v11;
		var v12:b2Vec2 = b2Collision.s_v12;
		...
		var clipPoints1:Array<ClipVertex> = b2Collision.s_clipPoints1;
		var clipPoints2:Array<ClipVertex> = b2Collision.s_clipPoints2;
		var np:number /** int */;
		...
		circle:b2CircleShape, xf2:b2Transform) : void
	{
		...
		var vertIndex1:number /** int */ = normalIndex;
		var vertIndex2:number /** int */ = vertIndex1 + 1 < vertexCount?vertIndex1 + 1:0;
