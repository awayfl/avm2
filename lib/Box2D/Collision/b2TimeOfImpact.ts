...
export class b2TimeOfImpact
{
	...
	public static TimeOfImpact(input:b2TOIInput):number
	{
		...
		for (;; )
		{
			...
			var separation:number = this.s_fcn.Evaluate(this.s_xfA, this.s_xfB);
			if (separation <= 0.0)
			{
				alpha = 1.0;
				break;
...
//#endif
			...
			var newAlpha:number = alpha;
			{
				...
				var f2:number = this.s_fcn.Evaluate(this.s_xfA, this.s_xfB);
				...
				var rootIterCount:number /** int */ = 0;
				for (;; )
				{
					...
					var f:number = this.s_fcn.Evaluate(this.s_xfA, this.s_xfB);
