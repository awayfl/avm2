...
export class b2DynamicTreeBroadPhase implements IBroadPhase
{
	...
	public UpdatePairs(callback:Function):void
	{
		...
		{
			var QueryCallback:Function = (proxy:b2DynamicTreeNode) =>
			{
			...
			var fatAABB:b2AABB = this.m_tree.GetFatAABB(queryProxy);
		...
		for (var i:number /**int */ = 0; i < this.m_pairCount; )
		{
			var primaryPair:b2DynamicTreePair = this.m_pairBuffer[i];
			var userDataA:any = this.m_tree.GetUserData(primaryPair.proxyA);
			var userDataB:any = this.m_tree.GetUserData(primaryPair.proxyB);
	...
	{
		var i:number /**int */ = this.m_moveBuffer.indexOf(proxy);
