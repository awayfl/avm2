...
export class b2World
{
	...
	*/
	...
	public DrawDebugData() : void{
		...
		var flags:number /** uint */ = this.m_debugDraw.GetFlags();
		...
		{
			for (var c:b2Controller = this.m_controllerList; c; c = c.m_next)
			{
	...
	*/
	...
	public Solve(step:b2TimeStep) : void{
		...
		for(var controller:b2Controller= this.m_controllerList;controller;controller=controller.m_next)
		{
		...
		var stackSize:number /** int */ = this.m_bodyCount;
		//b2Body** stack = (b2Body**)this.m_stackAllocator.Allocate(stackSize * sizeof(b2Body*));
		var stack:Array<b2Body> = this.s_stack;
	...
	public SolveTOI(step:b2TimeStep) : void{
		...
		var queue:Array<b2Body> = b2World.s_queue;
		...
		for (;;)
		{
			...
			island.Clear();
			var queueStart:number /** int */ = 0;	//start index for queue
			var queueSize:number /** int */ = 0;	//elements in queue
			...
			var subStep:b2TimeStep = b2World.s_timestep;
	...
	public DrawShape(shape:b2Shape, xf:b2Transform, color:b2Color) : void{
		...
		case b2Shape.e_polygonShape:
			{
				var i:number /** int */;
				var poly:b2PolygonShape = <b2PolygonShape> shape;
				var vertexCount:number /** int */ = poly.GetVertexCount();
