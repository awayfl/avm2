...
import { defineNonEnumerableProperty, release, assertNotImplemented, unexpected, isIndex } from '@awayfl/swf-loader';
...
export class Int32Vector extends BaseVector {
	...
	static classInitializer() {
		...
		const asProto: any = Int32Vector.prototype;
	...
	_ensureCapacity(length) {
		...
		const buffer = new Int32Array(newCapacity);
	...
	concat() {
		...
		for (let i = 0; i < arguments.length; i++) {
			const vector: Int32Vector = arguments[i];
		...
		const result = new this.sec.Int32Vector(length);
   ...
   */
	filter(callback, thisObject) {
		const v = new this.sec.Int32Vector();
		if (!this.checkVectorMethodArgs(callback, thisObject)) {
			return v;
		}
		for (let i = 0; i < this._length; i++) {
	...
	slice(start = 0, end = 0x7fffffff) {
		...
		const result = new this.sec.Int32Vector(last - first, this.fixed);
	...
	splice(start: number, deleteCount_: number /*, ...items: number[] */) {
		...
		const result = new this.sec.Int32Vector(deleteCount, this.fixed);
		if (deleteCount > 0) {
