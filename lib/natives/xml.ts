...
export class ASXML extends ASObject implements XMLType {
	...
	attributes(): ASXMLList {
		...
		const list = this.sec.AXXMLList.CreateList(this, this._name);
	...
	child(propertyName: any /* string|number|QName */): ASXMLList {
		...
		if (isIndex(propertyName)) {
			const list = this.sec.AXXMLList.CreateList(null, null);
	...
	inScopeNamespaces(): ASNamespace[] {
		...
		const namespaces = this._inScopeNamespacesImpl();
		const result = [];
		for (let i = 0; i < namespaces.length; i++) {
			const AXNamespace = this.sec.AXNamespace;
	...
	private _inScopeNamespacesImpl() {
		...
		let y: ASXML = this;
		// Step 2.
		const inScopeNS: Namespace[] = [];
		const inScopeNSMap = inScopeNS;
		// Step 3.
		while (y !== null) {
			// Step 3.a.
			const namespaces = y._inScopeNamespaces;
	...
	namespace(prefix?: string): any {
		...
		// Steps 1-3.
		const inScopeNS = this._inScopeNamespacesImpl();
		// Step 4.
		if (arguments.length === 0) {
			// Step 4.b.
	...
	replace(propertyName: any, value: any): ASXML {
		...
		// Step 4.
		const mn = toXMLName(propertyName, this.sec);
		if (isIndex(mn.name)) {
			this._replaceByIndex(mn.name|0, c);
			// Step 10.
			return this;
		}
		const isAnyName = mn.isAnyName();
		// Step 5 (Implicit).
		// Step 6.
		let i = -1;
		// Step 7.
	...
	setName(name_: any): void {
		...
		// Step 3.
		const name: Multiname = this.sec.AXQName.Create(name_).name;
		// Step 4.
	...
	setNamespace(ns: any): void {
		...
		// Step 2.
		const ns2 = this.sec.AXNamespace.Create(ns)._ns;
		// Step 3.
	...
	private toXMLStringImpl(ancestorNamespaces?: Namespace[], indentLevel?: number) {
		...
		const prettyPrinting = sec.AXXML.prettyPrinting;
		indentLevel |= 0;
		let s = prettyPrinting ? getIndentString(indentLevel) : '';

		const kind: ASXMLKind = node._kind;
		switch (kind) {
			// 4. If x.[[Class]] == "text",
			case ASXMLKind.Text:
		...
		if (namespace.prefix === undefined) {
			...
			// ns2.prefix
			const newPrefix = generateUniquePrefix(currentNamespaces);
			const ns2 = internPrefixedNamespace(NamespaceType.Public, namespace.uri, newPrefix);
		...
		for (let i = 0; i < namespaceDeclarations.length; i++) {
			...
			const attributeName = namespace.prefix ? 'xmlns:' + namespace.prefix : 'xmlns';
			s += ' ' +  attributeName + '="' + escapeAttributeValue(namespace.uri) + '"';
		}
		node._attributes && node._attributes.forEach(function (attr) {
			const name: Multiname = attr._name;
			const namespace = GetNamespace(name, ancestorNamespaces);
			const attributeName = namespace.prefix ? namespace.prefix + ':' + name.name : name.name;
		...
		// not equal to "text"))
		const indentChildren = node._children.length > 1 ||
	...
	setProperty(mn: Multiname, v: any) {
		...
		let i;
		const isAny = mn.isAnyName();
		const primitiveAssign = !isXMLType(c, sec) && !isAny && mn.name !== '*';
		const isAnyNamespace = mn.isAnyNamespace();
		...
		if (i === undefined) {
			...
			if (primitiveAssign) {
				...
				if (uri === null) {
					const defaultNamespace = getDefaultNamespace(sec);
	...
	getProperty(mn: Multiname): any {
		...
		// Step 2 (implicit).
		// Step 3.
		const list = this.sec.AXXMLList.CreateList(this, mn);
		let length = 0;
		const anyName = mn.isAnyName();
		const anyNamespace = mn.isAnyNamespace();
	...
	hasProperty(mn: Multiname): boolean {
		...
		const name = toXMLName(mn, this.sec);
...
function createXML(sec: AXSecurityDomain, kind: ASXMLKind = ASXMLKind.Text,
	...
	const xml = sec.AXXML.Create();
	const ns = internPrefixedNamespace(NamespaceType.Public, uri, prefix || '');
	const mn = new Multiname(null, 0,
...
export class ASXMLList extends ASObject implements XMLType {
	...
	static classInitializer() {
		...
		const proto: any = this.dPrototype;
		const asProto: any = ASXMLList.prototype;
	...
	public static axApply(_self: ASXMLList, args: any[]): ASXMLList {
		...
		const list = this.sec.AXXMLList.Create();
	...
	static Create(value?: any): ASXMLList {
		const list: ASXMLList = Object.create(this.sec.AXXMLList.tPrototype);
	...
	constructor (value?: any) {
		...
		if (value && value.axClass === this.sec.AXXMLList) {
			const children = (<ASXMLList>value)._children;
			for (let i = 0; i < children.length; i++) {
				const child = children[i];
	...
	static CreateList(targetObject: ASXML, targetProperty: Multiname) {
	...
	// 9.2.1.7 [[DeepCopy]] ( )
	_deepCopy() {
		const xl = this.sec.AXXMLList.CreateList(this._targetObject, this._targetProperty);
	...
	child(propertyName: any): ASXMLList {
		if (isIndex(propertyName)) {
			const list = this.sec.AXXMLList.CreateList(this._targetObject, this._targetProperty);
	...
	descendants(name_: any): ASXMLList {
		const name = toXMLName(name_, this.sec);
		const list = this.sec.AXXMLList.CreateList(this._targetObject, this._targetProperty);
		for (let i = 0; i < this._children.length; i++) {
			const child = this._children[i];
			if (child._kind === ASXMLKind.Element) {
	...
	parent(): any {
		...
		const parent = children[0]._parent;
	...
	append(V: any) {
		...
		if (V && V.axClass === this.sec.AXXMLList) {
			...
			const valueChildren: ASXML[] = V._children;
	...
	getProperty(mn: Multiname): any {
		...
		const isAnyName = nm === null || nm === '*';
		const isAnyNamespace = mn.isAnyNamespace();
		const isAttribute = mn.isAttribute();
		const xl = this.sec.AXXMLList.CreateList(this._targetObject, mn);
		const children = this._children;
		for (let i = 0; i < children.length; i++) {
			const v = children[i];
	...
	setProperty(mn: Multiname, value: any) {
		...
		if (isIndex(mn.name)) {
				...
				if (y._kind !== ASXMLKind.Attribute) {
					...
					if (r !== null) {
						...
						if (i > 0) {
							...
							const rLength = r._children.length - 1;
							for (j = 0; j < rLength; j++) {
          ...
          value._kind === ASXMLKind.Text || value._kind === ASXMLKind.Attribute) {
			...
			const childKind = currentChild._kind;
			const parent = currentChild._parent;
			// Step 2.e.
			if (childKind === ASXMLKind.Attribute) {
				const indexInParent = parent._children.indexOf(currentChild);
			...
			if (value && value.axClass === this.sec.AXXMLList) {
				// Step 2.f.i.
				const c = value._shallowCopy();
				const cLength = c._children.length;
	...
	axCallProperty(mn: Multiname, args: any []): any {
		const method = this[this.axResolveMultiname(mn)];
		// Check if the method exists before calling it.
		if (method) {
			validateCall(this.sec, method, args.length);
			return method.axApply(this, args);
