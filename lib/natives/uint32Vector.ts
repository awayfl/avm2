...
export class Uint32Vector extends BaseVector {
	...
	static classInitializer() {
		...
		const asProto: any = Uint32Vector.prototype;
	...
	_ensureCapacity(length) {
		...
		const buffer = new Uint32Array(newCapacity);
	...
	concat() {
		...
		const result = new this.sec.Uint32Vector(length);
   ...
   */
	filter(callback, thisObject) {
		const v = new this.sec.Uint32Vector();
		if (!this.checkVectorMethodArgs(callback, thisObject)) {
			return v;
		}
		for (let i = 0; i < this._length; i++) {
	...
	slice(start = 0, end = 0x7fffffff) {
		...
		const result = new this.sec.Uint32Vector(last - first, this.fixed);
	...
	splice(start: number, deleteCount_: number /*, ...items: number[] */) {
		...
		const result = new this.sec.Uint32Vector(deleteCount, this.fixed);
		if (deleteCount > 0) {
