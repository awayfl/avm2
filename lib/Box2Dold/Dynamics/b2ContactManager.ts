...
export class b2ContactManager extends b2PairCallback
{
	...
	public PairAdded(proxyUserData1:any, proxyUserData2:any):any{
		...
		if (createFcn != null)
		{
			...
			else
			{
				...
				for (var i:number /** int */ = 0; i < c.m_manifoldCount; ++i)
				{
	...
	public Destroy(c:b2Contact) : void
	{
		...
		if (manifoldCount > 0 && this.m_world.m_contactListener)
		{
			...
			for (var i:number /** int */ = 0; i < manifoldCount; ++i)
			{
				var manifold:b2Manifold = manifolds[ i ];
				cp.normal.SetV(manifold.normal);
				
				for (var j:number /** int */ = 0; j < manifold.pointCount; ++j)
				{
		...
		var type1:number /** int */ = c.m_shape1.m_type;
		var type2:number /** int */ = c.m_shape2.m_type;
	...
	public static InitializeRegisters() : void{
		...
		for (var i:number /** int */ = 0; i < b2Shape.e_shapeTypeCount; i++){
			b2Contact.s_registers[i] = new Array(b2Shape.e_shapeTypeCount);
			for (var j:number /** int */ = 0; j < b2Shape.e_shapeTypeCount; j++){
