...
export function compile(methodInfo: MethodInfo, sync = false): ICompilerProcess {
	...
	let o_lastKilledBoxedStack: string = undefined;

	const o_KillStack = (name: string, onlyBoxing = false) => {
		if(o_lastBoxedStack === name) {
			o_lastBoxedStack = undefined;
			o_lastKilledBoxedStack = name;
		}

		if(o_lastScopedStackAlias === name && !onlyBoxing) {
			o_lastScopedStackAlias = undefined;
		}
	}
			...
			switch (z.name) {
				...
				case Bytecode.GETSLOT:
					...
					if(UNSAFE_OPTIMISE_BOX) {
						...
						// yep, get slot kill boxing because overwrite stack
						o_KillStack(stack0);
						break;
					}
					js.push(`${idnt}                ${stack0} = sec.box(${stack0}).axGetSlot(${param(0)});`)
				...
				case Bytecode.GETSCOPEOBJECT:
					...
					if(UNSAFE_OPTIMISE_GET_SCOPED) {
						if(o_lastUsedScope === `scope${param(0)}`) {
							if(o_lastScopedStackAlias === stackN){
								js.push(`//${idnt}              Scoped object not changed between getting, skip assign`)
								js.push(`//${idnt}              ${stackN} = scope${param(0)}.object;`)
							} else {
								// try restore boxing
								// o_scoped can store location on already boxed object
								if(o_lastScopedStackAlias === o_lastKilledBoxedStack) {
									o_lastBoxedStack = o_lastScopedStackAlias;
									o_lastKilledBoxedStack = undefined;
									
									js.push(`//${idnt}              Boxing was restored from scoped object`)
									js.push(`${idnt}                ${stackN} = o_boxed = o_scoped;//scope${param(0)}.object;`)
								} else {
				...
				case Bytecode.NEWOBJECT:
					...
					o_KillStack(stackF(2 * param(0) - 1));
